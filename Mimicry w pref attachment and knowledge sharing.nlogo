...
;; which for our purposes we think of as amphora types:
;; monarchs are type X
;; viceroys are type Y
...
globals [
  ...
  carrying-capacity-persons     ;; maximum population of persons in the world
...
to setup
  clear-all
if random-seed? [random-seed 1066]
  setup-variables
  setup-turtles
  reset-ticks
...
to setup-variables
  ...
  set carrying-capacity-persons 75
  set reproduction-chance 4
  set color-range-begin 15
  set color-range-end 109
end

;; create 75 persons and 450 butterflies of which half are
...
to setup-turtles
  ...
  if network-type = "preferential attachment" 
   ...
   nw:generate-preferential-attachment persons links carrying-capacity-persons 
   [
    set color black
    set memory []
    set shape one-of ["person" "person farmer"]
   ]
   ]
  
  if network-type = "small world"
   [nw:generate-small-world persons links carrying-capacity-persons / 8 carrying-capacity-persons / 9 2.0 false  ;;kludge to get network with n=75
    [
    set color black
    set memory []
    set shape one-of ["person" "person farmer"]
...
to go
  ask persons [ persons-move ]
  ;; turtles that are not persons are butterflies
  ask turtles with [breed != persons] [ butterflies-move ]
  ask turtles with [breed != persons] [ butterflies-get-eaten ]
  ask persons [ persons-forget ]
  ask turtles with [breed != persons] [ butterflies-reproduce ]
  tick
end

to persons-move ;; persons procedure
  ;; The persons are animated by alternating shapes
;;  ifelse shape = "person"
;;  [ set shape "bird"]
;;  [ set shape "person" ]
...
;; If there is a person on this patch check the person's memory
;; to see if this butterfly seems edible based on the color.
;; If the butterfly's color is not in the person's memory
;; the butterfly dies.  If it's a monarch the person remembers
;; that its color was yucky
to butterflies-get-eaten  ;; butterfly procedure
  ifelse share-knowledge?
  [
  let person-here one-of persons-here
  if person-here != nobody
  [ let other-person one-of [link-neighbors] of person-here
    if other-person != nobody
   [ 
    if not [color-in-memory? [color] of myself] of person-here
     or
     not [color-in-memory? [color] of myself] of other-person
    [
      if breed = monarchs
        [ ask person-here [ remember-color [color] of myself ] ]
      die
    ]
  ]
  ]
...
;; color is in a person's memory
to-report color-in-memory? [c] ;; person procedure
  foreach memory [ if item 0 ? = c [ report true ] ]
  report false
end

;; put a color that was yucky in memory
to remember-color [c]  ;; person procedure
  ;; persons can only remember 3 colors at a time
...
;; persons can only remember for so long, then
;; they forget. They remember colors for MEMORY-LENGTH
to persons-forget ;; person procedure
...
"" ""
PENS
"Type X" 1.0 0 -2674135 true "" "plot mean [color] of monarchs"
"Type O" 1.0 0 -13345367 true "" "plot mean [color] of viceroys"
...
@#$#@#$#@
<experiments>
  <experiment name="pref attach w share-knowledge" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    <metric>mean [color] of monarchs</metric>
    <metric>mean [color] of viceroys</metric>
    ...
    <enumeratedValueSet variable="share-knowledge?">
      <value value="true"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="run without sharing knowledge" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    <metric>mean [color] of monarchs</metric>
  ...
  <experiment name="small world w knowledge share" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    <metric>mean [color] of monarchs</metric>
