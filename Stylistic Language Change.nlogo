...
to update-color
  set color scale-color red state 0 2
end
...
to save-plots
  let y random 1000
  let x "C:\\Users\\Shawn Graham\\Dropbox\\abm\\grammar\\"
  let z "plots.csv"
  let part1 word x y
  let settings word network-type threshold-val
...
43
go
go\nif count nodes with [color = 10] = num-nodes [stop]\nif count nodes with [color = red] = num-nodes [stop]
T
1
T
OBSERVER
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [state = 0.0]"
"pen-1" 1.0 0 -2674135 true "" "plot count turtles with [state = 1.0]"
"pen-2" 1.0 0 -7500403 true "" "plot count turtles with [state = 2.0]"
...
* Troutman, C. and Wilensky, U. (2007).  NetLogo Language Change model.  http://ccl.northwestern.edu/netlogo/models/LanguageChange.  Center for Connected Learning and Computer-Based Modeling, Northwestern Institute on Complex Systems, Northwestern University, Evanston, IL.
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>save-plots</final>
    <timeLimit steps="200"/>
    <metric>count turtles with [state = 0.0]</metric>
    <metric>count turtles with [state = 1.0]</metric>
    <metric>count turtles with [state = 2.0]</metric>
    ...
    <steppedValueSet variable="threshold-val" first="0" step="0.1" last="0.9"/>
